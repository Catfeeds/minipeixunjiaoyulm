	...
	*/
	public function index(){
		...
		foreach ($adv_list as $k => $v) {
			if ($v['type']=='cat') {
			    $adv_list[$k]['desc'] = '分类页';
			}else if ($v['type']=='procat') {
			    $adv_list[$k]['desc'] = '分类商品列表页';
			}else if ($v['type']=='prolist') {
			    $adv_list[$k]['desc'] = '商品列表页';
			}else if ($v['type']=='pro') {
			    $adv_list[$k]['desc'] = '商品详情页';
			}else if ($v['type']=='newslist') {
			    $adv_list[$k]['desc'] = '新闻资讯列表页';
			}else if ($v['type']=='news') {
			    $adv_list[$k]['desc'] = '新闻资讯详情页';
			}else if ($v['type']=='shoplist') {
			    $adv_list[$k]['desc'] = '商铺列表页';
			}else if ($v['type']=='shop') {
			    $adv_list[$k]['desc'] = '商铺详情页';
			}else if ($v['type']=='join') {
			    $adv_list[$k]['desc'] = '加盟合作页';
			}else if ($v['type']=='life') {
			    $adv_list[$k]['desc'] = '生活专区页';
			}else if ($v['type']=='vou') {
			    $adv_list[$k]['desc'] = '优惠券列表页';
			}else if ($v['type']=='courselist') {
			    $adv_list[$k]['desc'] = '活动列表页';
			}else if ($v['type']=='course') {
			    $adv_list[$k]['desc'] = '活动详情页';
			}else {
				$adv_list[$k]['desc'] = '首页';
			}
		}
