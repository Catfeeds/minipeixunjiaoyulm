...
class ShangchangController extends PublicController{
	...
	public function add(){	
		...
		if($_POST['submit']==true){
		   ...
		   $array=array(
				'name' => $_POST['name'] ,
				'uname' => $_POST['uname'] ,
				'per_price' => floatval($_POST['per_price']),
	...
	public function setshop(){
		...
		if ($_POST['submit']==true) {
			...
			$up['pwd']=md5(md5($pwd));
			$up['name'] = $name;
			$up['uname'] = M('shangchang')->where('id='.intval($shop_id))->getField('name');

			//检测登录账号
			$check = M('shopuser')->where('name="'.$name.'" AND del=0')->find();
			...
			if ($user_info) {
				if ($check && intval($check['shop_id'])!=intval($shop_id)) {
					$this->error('登录账号已经存在.'.__LINE__);
					die();
				}
				$res = M('shopuser')->where('shop_id='.intval($shop_id))->save($up);
			}else{
				if ($check) {
					$this->error('登录账号已经存在.'.__LINE__);
					die();
				}
				$up['shop_id'] = intval($shop_id);
				$up['addtime'] = time();
				$res = M('shopuser')->add($up);
			}
		...
		$shop_info = M('shopuser')->where('shop_id='.intval($_REQUEST['shop_id']).' AND del=0')->find();
